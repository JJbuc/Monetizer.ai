...
- **Fallback**: Direct Claude API integration
- **Features**:
...
- **TTS (Text-to-Speech)**: ElevenLabs API (`sk_e1774e11047122dc1fad8b76a6e3df35d0a86fa1593fa04c`)
...
- Avatar video (`avatar/marcus brownlee.mp4`)
...
4. Display success modal after 500ms delay
...
### Scalability
- Horizontal scaling for Flask servers
...
- **Backend**: AWS EC2 or AWS Amplify
...
2. **Frontend Layer**: HTML/CSS/JS files served via CDN
3. **API Gateway**: Flask server handling requests
4. **Business Logic**: RAG service, AI integration, Voice processing
5. **Data Layer**: Supabase databases per creator
6. **External Services**: Groq API, ElevenLabs API
7. **Data Flow Arrows**: Show request/response directions
8. **Caching Layer**: (Optional) Redis for session management
9. **Load Balancer**: For horizontal scaling
10. **Security Layer**: CORS, HTTPS, API key management
